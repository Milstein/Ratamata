...
namespace AspxCommerce.Core
{
    public class EmailTemplate
    {
        ...
        public static void SendEmailForReservation(AspxCommonInfo aspxCommonObj, string senderName, string receiverEmailID, string message, string messageBodyDetail)
        {
            StoreSettingConfig ssc = new StoreSettingConfig();
            string logosrc = ssc.GetStoreSettingsByKey(StoreSetting.StoreLogoURL, aspxCommonObj.StoreID,
                                                       aspxCommonObj.PortalID, aspxCommonObj.CultureName);
            List<MessageManagementInfo> template =
                MessageManagementController.GetMessageTemplateByMessageTemplateTypeID(
                    SystemSetting.FLIGHT_RESERVATION_EMAIL, aspxCommonObj.PortalID);
            var sub = "";
            string senderEmail = "";
            ...
            foreach (MessageManagementInfo messageToken in template)
            {
                ...
                if (messageToken != null)
                {
                    ...
                    sub = messageToken.Subject.Replace("%FlightType%", message);
                    sub = sub.Replace("%UserEmail%", senderName);
                    senderEmail = messageToken.MailFrom;
                }
                ...
                emailSuperAdmin = pagebase.GetSettingsByKey(SageFrameSettingKeys.SuperUserEmail);
                emailSiteAdmin = pagebase.GetSettingsByKey(SageFrameSettingKeys.SiteAdminEmailAddress);
                MailHelper.SendMailNoAttachment(senderEmail, receiverEmailID, sub, messageTemplate, emailSiteAdmin, emailSuperAdmin);
