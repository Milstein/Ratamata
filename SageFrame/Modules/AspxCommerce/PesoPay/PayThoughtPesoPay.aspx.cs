...
public partial class Modules_AspxCommerce_PesoPay_PayThoughtPesoPay : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (Session["PesoPayData"] != null)
            {
                string[] data = Session["PesoPayData"].ToString().Split('#');
        ...
        try
        {
            ...
            if (bool.Parse(sf.PesoPayIsSecureHashEnabled))
            {
                ...
                secureHash = sha1.generatePaymentSecureHash(
                    ...
                    orderdata2.ObjOrderDetails.InvoiceNumber,
