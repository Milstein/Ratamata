<%@ Control Language="C#" AutoEventWireup="true" CodeFile="PesoPayView.ascx.cs" Inherits="PesoPayView" %>
...
<script type="text/javascript">
    ...
    $(function() {
        var paymentMethodName = "PesoPay";
        var paymentMethodCode = "";
        var pesoPayPaymentType = 0;
        var ref = "";
        var aspxCommonObj = {
...
//                        AspxCommerce.Busy.LoadingHide();
            ...
            init: function() {
                $('#btnPesoPay').live("click", function() {
                    ...
                    if ($('#SingleCheckOut').length > 0) {
                        ...
                        AspxOrder.SendDataForPaymentPesoPay();
                    }
                });
                $('#txtCardNumber').bind("change", function() {
                    AspxOrder.CheckCreditCard();
                });
                AspxOrder.LoadSetting();
            },
            CheckCreditCard: function() {
                this.config.method = "AspxCommerceWebService.asmx/CheckCreditCard";
                this.config.url = this.config.baseURL + this.config.method;
                this.config.data = JSON2.stringify({ storeID: AspxCommerce.utils.GetStoreID(), portalID: AspxCommerce.utils.GetPortalID(), cultureName: AspxCommerce.utils.GetCultureName(), creditCardNo: $.trim($('#txtCardNumber').val()) });
                this.config.ajaxCallMode = 5;
            ...
            LoadSetting: function() {
                ...
                $.ajax({
                    type: "POST",
                    async: false,
                    url: '<%=PathPesoPay%>' + "Services/PesoPayWebService.asmx/GetPesoPaySettings",
                    data: JSON2.stringify({ storeId: AspxCommerce.utils.GetStoreID(), paymentGatewayId: paymentgateWayId, portalId: AspxCommerce.utils.GetPortalID() }),
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
                    success: function(data) {
                        var value = eval(data.d);
                        if (value.PesoPayPaymentType == 1) {
                            $("#divCrediCardForm").hide();
                            $("#btnPesoPay").removeAttr('paymentType').attr('paymentType', value.PesoPayPaymentType);

                        } else {
                            $("#divCrediCardForm").show();
                            $("#btnPesoPay").removeAttr('paymentType').attr('paymentType', value.PesoPayPaymentType);

                        }
                        paymentMethodCode = value.PesoPayPaymentMethod;
                        pesoPayPaymentType = value.PesoPayPaymentType;
                    },
                    error: function() {
                        csscody.alert("<h2>Information Alert</h2><p>Failed to load PesoPay.!!</p>");
                        return false;
                    }
                });
            ...
            ajaxSuccess: function(data) {
                switch (AspxOrder.config.ajaxCallMode) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        if ($.trim(pesoPayPaymentType) == 3) {
                            document.location = aspxRedirectPath + "PesoPay-Success.aspx?Ref=" + ref;
                        } else {
                            ...
                            for (var i = 0; i < CheckOut.UserCart.lstItems.length; i++) {
                                itemIds += CheckOut.UserCart.lstItems[i].ItemID + "N" + CheckOut.UserCart.lstItems[i].Quantity + ",";

                            }
                            x = x + "#" + itemIds + "#" + CheckOut.UserCart.couponCode + "N" + couponApplied;

                            AspxOrder.SetSessionValue("PesoPayData", x);
                            document.location = '<%=PathPesoPay%>' + "PayThoughtPesoPay.aspx";
                        }
                        break;
                    case 4:
                        if ($.trim(pesoPayPaymentType) == 3) {
                    ...
                    case 5:
                        if (data.d) {
                            //blocked cc
                            csscody.alert("<h2>Information Alert</h2><p>Your credit card is blacklisted!!</p>");
                            $('#txtCardNumber').val('');
                            if ($('#SingleCheckOut').length > 0) {
            ...
            SendDataForPaymentPesoPay: function() {
//                AspxCommerce.Busy.LoadingShow();
                ...
                var cardCode = $('#txtSecurityCode').val();
                var cardType = $('#cardType').val();
                var cardHolderName = $('#txtCardHolder').val();

                var expireMonth = $('#ddlExpiryMonth option:selected').text();
                var expireYear = $('#ddlYear option:selected').text();
                ...
                var OrderDetails = {
                    ...
                    OrderDetailsInfo: {
                        ...
                        TransactionID: 0,
                        GrandTotal: amount,
                        DiscountAmount: CheckOut.UserCart.TotalDiscoun,
                ...
                if ($.trim(pesoPayPaymentType) == 3) {
                    var d1 = new Date();
                    var invoice = d1.format('yyyyMMddhhmmss').toString(); ref = invoice;
                    paramData.OrderDetailsCollection.ObjOrderDetails.InvoiceNumber = invoice;
                    $.ajax({
                        type: "POST",
                        url: '<%=PathPesoPay%>' + "Services/PesoPayWebService.asmx/DirectServerPost",
                        data: JSON2.stringify({ "orderDetails": paramData.OrderDetailsCollection }),
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function(data) {
                            var response = eval(data.d);
                            if ($.trim(response.SuccessCode) == "0") {
                                csscody.info("<h2>Success Message</h2><p>Your Payment has been approved.!!</p>");
                                paramData.OrderDetailsCollection.ObjOrderDetails.OrderStatusID = 8;
                                paramData.OrderDetailsCollection.ObjOrderDetails.TransactionID = response.PayRef;
                                paramData.OrderDetailsCollection.ObjOrderDetails.ResponseReasonText = "Transaction completed Successfully";
                                AspxOrder.config.method = "AspxCommerceWebService.asmx/SaveOrderDetails";
                                AspxOrder.config.url = AspxOrder.config.baseURL + AspxOrder.config.method;
                                AspxOrder.config.data = JSON2.stringify({ "orderDetail": paramData.OrderDetailsCollection });
                                AspxOrder.config.ajaxCallMode = 4;
                                AspxOrder.config.error = 4;
                                AspxOrder.ajaxCall(AspxOrder.config);
                            } else {
                                csscody.alert("<h2>Information Alert</h2><p>" + response.ErrMsg + ".Please Re-Submit your information!!</p>");
//                                AspxCommerce.Busy.LoadingHide();
                            }
                        ...
                        error: function() {
                            csscody.alert("<h2>Information Alert</h2><p>Failed to save orer.!</p>");
//                            AspxCommerce.Busy.LoadingHide();
                        }
                    });
                } else {
...
//                AspxCommerce.Busy.LoadingShow();
                ...
                if ($.trim(pesoPayPaymentType) == 3) {
                    var d1 = new Date();
                    var invoice = d1.format('yyyyMMddhhmmss').toString();
                    ref = invoice;
                    paramData.OrderDetailsCollection.ObjOrderDetails.InvoiceNumber = invoice;
                    $.ajax({
                        type: "POST",
...
<div id="divCrediCardForm" style="display: none;" class="cssClassCardInfobox" >
    <h1>
        Enter Card Information</h1>
    <asp:Label ID="lblGeneralInfo" runat="server" Text="Your information will not be stored and are passed through secure connection."></asp:Label>
    <table>
        <tr>
            <td>
                <asp:Label ID="lblCardHolder" runat="server" Text="">Card Holder:<span class="cssClassRequired">*</span></asp:Label>
            </td>
            <td>
                <input type="text" id="txtCardHolder" name="cardholder" class=" required"/>
            </td>
        </tr>
        <tr>
            <td>
                <asp:Label ID="lblPaymentMethod" runat="server" Text="">PaymentMethod:<span class="cssClassRequired">*</span></asp:Label>
            </td>
            <td><select id="cardType">
                    <option selected="selected" value="VISA" >VISA</option>
                      <option  value="Master Card" >Masters</option>
                        <option  value="dnr" >Diners</option>
        ...
        <tr>
            <td>
                <asp:Label ID="lblExpiryMonth" runat="server" Text="">ExpiryMonth:<span class="cssClassRequired">*</span></asp:Label>
            </td>
            <td>
            ...
            <td>
                <asp:Label ID="lblExpiryYear" runat="server" Text="">Expiry Year:<span class="cssClassRequired">*</span></asp:Label>
            </td>
            <td>
                <select id="ddlYear">
        ...
        <tr>
            <td>
                <asp:Label ID="lblCardNumber" runat="server" Text="">Card Number:<span class="cssClassRequired">*</span></asp:Label>
            </td>
            <td>
                <input type="text" id="txtCardNumber" maxlength="16" size="22" class="creditcard  required"
        name="creditCard" />
            </td>
        </tr>
        <tr>
            <td>
                <asp:Label ID="lblSecurityCode" runat="server" Text="">Security Code:<span class="cssClassRequired">*</span></asp:Label>
            </td>
            <td>
                <input type="text" id="txtSecurityCode"  size="10" maxlength="4" name="cardCode" class=" required" minlength="3"/>
            </td>
        </tr>
        <tr>
            <td>
...
<input type="button" id="btnPesoPay" value="Place Order (PesoPay)" />
