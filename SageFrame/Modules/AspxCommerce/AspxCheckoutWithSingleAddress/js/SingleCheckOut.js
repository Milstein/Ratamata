        ...
        var CheckOut = {
            ...
            CheckShippingAndBillingCountry: function (checktype) {
                ...
                if ("anonymoususer" != userName) {
                    billing = $('input[name="billing"]:checked').text().split(',')[4];
                    shipping = $('input[name="shipping"]:checked').text().split(',')[4];
                } else {
            ...
            BindPaymentGateWayList: function (data) {
                if (data.d.length > 0) {
                    ...
                    $.each(data.d, function (index, item) {
                        if (item.LogoUrl != '') {
                            $('#dvPGList').append('<label><input id="rdb' + item.PaymentGatewayTypeName + '" name="PGLIST" type="radio" realname="' + item.PaymentGatewayTypeName + '" friendlyname="' + item.FriendlyName + '"  source="' + item.ControlSource + '" value="' + item.PaymentGatewayTypeID + '" class="cssClassRadioBtn" /><img class="cssClassImgPGList" src="' + aspxRootPath + item.LogoUrl + '" alt="' + item.PaymentGatewayTypeName + '" height="30" width="100" title="' + item.PaymentGatewayTypeName + '" visible="true" /></label>');
                        } else {
            ...
            Init: function () {
                ...
                $("#ddlSPCountry ,#ddlBLCountry ,input:radio[name=billing]").bind("change", function () {
                    if ($("#chkBillingAsShipping").is(":checked")) {
