...
$(function () {
    ...
    FrontGallery = {
        ...
        BindFeaturedItemsGallery: function () {
            ...
            if (parseInt(AspxCommerce.utils.GetCustomerID()) > 0) {
                this.config.method = "AspxPersonalization/AspxPersonalizationWebService.asmx/GetPersonFeaturedItems";
                var servicePath = this.config.baseURL;
                servicePath = servicePath.replace('AspxCommerceServices/', '');
                this.config.url = servicePath + this.config.method;
            }
            else {
        ...
        BindSpecialItemsGallery: function () {
            ...
            if (parseInt(AspxCommerce.utils.GetCustomerID()) > 0) {
                this.config.method = "AspxPersonalization/AspxPersonalizationWebService.asmx/GetPersonSpecialItemsGallery";
                var servicePath = this.config.baseURL;
                servicePath = servicePath.replace('AspxCommerceServices/', '');
                this.config.url = servicePath + this.config.method;
            }
            else {
                this.config.method = "AspxCommerceWebService.asmx/GetSpecialItemsList";
                this.config.url = this.config.baseURL + this.config.method;
            }
        ...
        LoadFeatureItemRssImage: function () {
            ...
            $('#featureItemRssImage').parent('a').removeAttr('href').attr('href', pageurl + '?type=rss&action=featureitems'); //featureitems for feature items
        ...
        LoadSpecialItemRssImage: function () {
            var pageurl = aspxRedirectPath + rssFeedUrl + pageExtension;
            $('#featureItemRssImage').parent('a').show();
            $('#featureItemRssImage').parent('a').removeAttr('href').attr('href', pageurl + '?type=rss&action=specialitems');
            $('#featureItemRssImage').removeAttr('src').attr('src', aspxTemplateFolderPath + '/images/rss-icon.png');
            $('#featureItemRssImage').removeAttr('title').attr('title', getLocale(AspxRssFeedLocale, "Special Items Rss Feed Title"));
            $('#featureItemRssImage').removeAttr('alt').attr('alt', getLocale(AspxRssFeedLocale, "Special Items Rss Feed Alt"));
        },
        Init: function () {
            ...
            if (GalleryDisplayAs == "Featured") {
                ...
                if (specialItemRss.toLowerCase() == 'true') {
                    FrontGallery.LoadSpecialItemRssImage();
                }
            }
        }
    };
