...
/// for Service tab
   ...
   function GetStoreLocation() {
    ...
    $.ajax({
        type: "POST",
        url: aspxservicePath + "AspxCommerceWebService.asmx/GetAllStoresForService",        data: JSON2.stringify({aspxCommonObj:aspxCommonInfo, serviceCategoryId:null}),
...
//             $tabs.tabs('option', 'active', 0);
         ...
         GetCategoryCkeckedItems:function(categoryID){
             ...
             this.config.data = JSON2.stringify({ CategoryID:categoryID, aspxCommonObj: aspxCommonObj() });
         ...
         BindCategoryItemsGrid: function(categoryId, itemSKU, itemName, itemPriceFrom, itemPriceTo, serviceBit) {
             ...
             UserDashBoardCategoryMgmt.GetCategoryCkeckedItems(categoryId);
         },
                ...
                $(this).val(Encoder.htmlEncode($(this).val()));
                 ...
                 for (var i = 0; i < editorList.length; i++) {
                     ...
                     textArea.val(Encoder.htmlEncode(editorList[i].Editor.getData()));
                                 ...
                                 var catNameTxtBoxID = UserDashBoardCategoryMgmt.variables.hdnCatNameTxtBox;                 var CatName = $("#" + catNameTxtBoxID).val();
                                 if (VerifyAvailabilityForm()) {
                     if (checkForm(frm) && UserDashBoardCategoryMgmt.CheckUniqueness(CatName)) {                         UserDashBoardCategoryMgmt.SaveCategory("#" + frmID);
                     } else {
...
//                     $tabs.tabs('select', 5);
         ...
         CreateTabPanel: function(attGroup) {
             ...
             dynHTML += '<div id="CategorManagement_TabContainer" class="responsive-tabs responsiveCAtmgr">';
//             dynHTML += '<ul>';
             for (var i = 0; i < attGroup.length; i++) {
                 tabs += '<h2><a href="#CategoryTab-' + attGroup[i].key + '"><span>' + attGroup[i].value + '</span></a></h2>';
                 tabs += '<div id="CategoryTab-' + attGroup[i].key + '"><table width="100%" border="0" cellpadding="0" cellspacing="0">' + attGroup[i].html + '</table></div>';
             }
             tabs += '<h2><a href="#CategoryTab-' + eval(attGroup.length + 1) + '"><span>Category Products</span></a></h2>';

             tabs += '<div id="CategoryTab-' + eval(attGroup.length + 1) + '">';

             tabs += '<div class="cssClassCommonBox Curve">';
             tabs += '<div class="sfGridwrapper"><div class="sfGridWrapperContent"><div id="ItemSearchPanel" class="cssClassSearchPanel sfFormwrapper"></div><div class="loading"><img src="' + aspxTemplateFolderPath + '/images/ajax-loader.gif" /></div><div class="log"></div>';
             tabs += '<table id="gdvCategoryItems" cellspacing="0" cellpadding="0" border="0" width="100%"></table></div></div>';
             tabs += '</div></div>';

                         tabs += '<h2 class="cssClassService"><a href="#CategoryTab-' + eval(attGroup.length + 2) + '"><span>Availability</span></a></h2>';

             tabs += '<div id="CategoryTab-' + eval(attGroup.length + 2) + '">';

             GetStoreLocation();
                         tabs += ' <div><div class="sfButtonwrapper"><button type="button" class="cssClassAddProvider" onclick="AddServiceProviderToBranch(this);">Manage Provider</button></div><div class="sfGridwrapper" id="divServiceInfo">';
             tabs += '<table  cellspacing="0" cellpadding="0" border="0" width="100%">';
             tabs += '<thead><tr class="cssClassHeading"><th>Pos</th><th>Branch</th><th> Service Provider Availability</th><th></th><th></th></tr></thead>';
             tabs += '<tbody><tr class="servicerow serviceTr_1"><td><input size="3" class="serviceId" value="0" type="hidden"><input size="3" class="cssClassDisplayOrder" type="text" value="1" disabled="disabled"></td><td><select class="cssClassDdlStoreCollection"><option>--</option></select><span class="cssBranchError" style="color: red;"></span></td>';

             tabs += '<td><div class="cssClassProviderParentDiv"><div class="cssClassServceProvider"><input size="3" class="serviceEmployeeId" value="0" type="hidden"><select class="cssClassDdlServiceProviders"><option>--</option></select><span class="cssProviderError" style="color: red;"></span>';//<button type="button" class="cssClassAddProvider" onclick="AddServiceProviderToBranch(this);">Add Provider</button>';
             tabs += '<p class="cssServiceProvidersSpan"><span class="cssServiceData cssAssignServiceProvider"  onclick="AssignServiceEmployee(this)"> <img  src="' + aspxTemplateFolderPath + '/images/admin/btnadduser.png"/>Assign Provider</span>';
                                   tabs += '<span class="cssServiceData cssClassDeleteAssingProvider"  onclick="DeleteAssignServiceProvider(this)"> <img  src="' + aspxTemplateFolderPath + '/images/admin/commondeletebtn.png"/>Delete Provider</span></p>';
             tabs += '<div class="cssClassDate"><ul class="cssClassUlDate"><li class="cssClassLiDate"><input size="3" class="serviceDateId" value="0" type="hidden"><label for="from">From:</label><input type="text" name="serviceAvailableDate" class="cssClassServiceAvailableDateFrom sfInputbox"/><label for="to">To:</label><input type="text"  name="cssClassServiceAvailableDateTo" class="cssClassServiceAvailableDateTo sfInputbox"/>';
                         tabs += '<span class="cssServiceProviderDate"><p class="cssServiceData cssClassAddDateBtn" onclick="AddServiceDate(this);"><img src="' + aspxTemplateFolderPath + '/images/admin/imgadd.png" />Add Date</p><p class="cssServiceData cssClassDeleteServiceDate" onclick="DeleteServiceDate(this);"><img src="' + aspxTemplateFolderPath + '/images/admin/commondeletebtn.png" />Delete Date</p></span>';
             tabs += '<div class="cssClassTime_1"><span class="cssClassSpanTime">';
             tabs += '<input size="3" class="serviceTimeId" value="0" type="hidden"><label for="fromTime">From:</label><input type="text" name="time" value="" class="sfInputbox timeFrom"/>';
             tabs += '<label for="toTime">To:</label><input type="text" name="time" value="" class="sfInputbox timeTo"/>';

                      tabs += '<label class="cssServiceProviderTime"><p class="cssServiceData" onclick="AddServiceTime(this);"><img src="' + aspxTemplateFolderPath + '/images/admin/imgadd.png" />Add Time</p><p class="cssServiceData" onclick="DeleteServiceTime(this);"><img src="' + aspxTemplateFolderPath + '/images/admin/commondeletebtn.png" />Delete Time</p></label>';
           
                         tabs += '</td>';
             tabs += '<td><p class="cssServiceData cssClassAddStoreBtn" onclick="AddMoreBranchService(this)"><img src="' + aspxTemplateFolderPath + '/images/admin/imgadd.png"/>Add Row</p></td>';
                                    tabs += '<td><p class="cssServiceData cssClassDeleteStoreBtn" onclick="DeleteServiceRow(this)"><img  src="' + aspxTemplateFolderPath + '/images/admin/commondeletebtn.png" />Delete Row</p></td>';
             tabs += '</tr></tbody>';
             tabs += '</table>';
             tabs += '<span class="cssClassNote">Note: Later added date overlap previous same date value of same store.</span></div></div>';
                         tabs += '</div>';
             ...
             switch (UserDashBoardCategoryMgmt.config.ajaxCallMode) {
             	...
             	case 7:
                     ...
                     $("#gdvCategoryItems .categoryCheckBox").each(function() {
                     	...
                     	checkedItem.IsCheckedID = $(this).prop('checked');
