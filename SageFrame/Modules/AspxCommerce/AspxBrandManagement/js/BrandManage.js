...
$(function() {
    ...
    BrandManage = {
        ...
        CheckBrandUniqueNess: function(brandName) {
            var param = JSON2.stringify({ brandName: brandName, aspxCommonObj: aspxCommonObj() });
        ...
        InsertBrand: function(prevFilePath, defaultImageProductURL, brandID, brandName, description, isShowInSlider, isFeatured, featuredFrom, featuredTo) {
            ...
            var param = JSON2.stringify({ prevFilePath: prevFilePath, aspxCommonObj: aspxCommonObj(), brandInsertObj: brandInsertObj });
            ...
            var upload = new AjaxUpload($('#txtBrandImageUrl'), {
                ...
                onSubmit: function(file, ext) {
                    if (ext != "exe") {
                        if (ext && /^(jpg|jpeg|jpe|gif|bmp|png|ico)$/i .test(ext)) {
                            ...
                            csscody.alert('<h1>' + getLocale(Brand, 'Alert Message') + "</h1><p>" + getLocale(Brand, 'Not a valid image!') + '</p>');
                            return false;
                        }
                    } else {
                        csscody.alert('<h1>' + getLocale(Brand, 'Alert Message') + "</h1><p>" + getLocale(Brand, 'Not a valid image!') + '</p>');
                ...
                onComplete: function(file, response) {
                    ...
                    if (res.Message != null && res.Status > 0) {
                        ...
                        csscody.error('<h1>' + getLocale(Brand, "Error Message") + "</h1><p>" + getLocale(Brand, 'Can not upload the image!') + '</p>');
                        return false;
                    }
                }
            });
        ...
        Init: function() {
            ...
            $("#btnDeleteSelected").click(function() {
                ...
                csscody.confirm("<h1>" + getLocale(Brand, "Delete Confirmation") + "</h1><p>" + getLocale(Brand, "Do you want to delete?") + "</p>", properties);
            });
            $("#btnSave").bind('click', function() {
                ...
                if (brandName == "") {
                    csscody.alert('<h1>' + getLocale(Brand, 'Alert Message') + "</h1><p>" + getLocale(Brand, 'Please enter brand name!') + '</p>');
                    return false;
                }
                if (isFeatured == true) {
                    ...
                    if (featuredFrom == "" || featuredTo == "") {
                        csscody.alert('<h1>' + getLocale(Brand, 'Alert Message') + "</h1><p>" + getLocale(Brand, 'Please enter valid date!') + '</p>');
                        return false;
                    }
                }
                if (defaultImageProductURL != "") {
                    ...
                    csscody.alert('<h1>' + getLocale(Brand, 'Alert Message') + '</h1><p>' + getLocale(Brand, 'Choose Image!') + '</p>');
                    return false;
                }
            });
